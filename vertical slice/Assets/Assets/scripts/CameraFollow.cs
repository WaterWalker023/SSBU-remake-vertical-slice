...
public class CameraFollow : MonoBehaviour
{
    ...
    public FocusOnLevel MapFocusPoint;
    ...
    void Start()
    {
        Players.Add(MapFocusPoint.gameObject);
    }
    ...
    private void CalculateCameraLocations()
    {
        ...
        for (int i = 0; i < Players.Count; i++)
        {
            ...
            if (!MapFocusPoint.FocusBounds.Contains(playerPosition))
            {
                float playerX = Mathf.Clamp(playerPosition.x, MapFocusPoint.FocusBounds.min.x, MapFocusPoint.FocusBounds.max.x);
                float playerY = Mathf.Clamp(playerPosition.y, MapFocusPoint.FocusBounds.min.y, MapFocusPoint.FocusBounds.max.y);
                float playerZ = Mathf.Clamp(playerPosition.z, MapFocusPoint.FocusBounds.min.z, MapFocusPoint.FocusBounds.max.z);
        ...
        float lerpPercent = Mathf.InverseLerp(0, (MapFocusPoint.HalfXBounds + MapFocusPoint.HalfYBounds) / 2, extents);
